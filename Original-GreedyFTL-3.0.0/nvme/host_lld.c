...
void set_nvme_admin_queue(unsigned int sqValid, unsigned int cqValid,
...
unsigned int get_nvme_cmd(unsigned short *qID, unsigned short *cmdSlotTag,
		unsigned int *cmdSeqNum, unsigned int *cmdDword) {
...
void set_auto_nvme_cpl(unsigned int cmdSlotTag, unsigned int specific,
	...
	//xil_printf("set nvme_auto_cpl %u\r\n");
...
void set_nvme_slot_release(unsigned int cmdSlotTag) {
	...
	//xil_printf("set_nvme_slot_release_cpl %u\r\n");
	IO_WRITE32(NVME_CPL_FIFO_REG_ADDR, nvmeReg.dword[0]);
	//IO_WRITE32((NVME_CPL_FIFO_REG_ADDR + 4), nvmeReg.dword[1]);
	IO_WRITE32((NVME_CPL_FIFO_REG_ADDR + 8), nvmeReg.dword[2]);
}

void set_nvme_cpl(unsigned int sqId, unsigned int cid, unsigned int specific,
		unsigned int statusFieldWord) {
	NVME_CPL_FIFO_REG nvmeReg;
...
void set_io_sq(unsigned int ioSqIdx, unsigned int valid, unsigned int cqVector,
		unsigned int qSzie, unsigned int pcieBaseAddrL,
...
void set_io_cq(unsigned int ioCqIdx, unsigned int valid, unsigned int irqEn,
		unsigned int irqVector, unsigned int qSzie, unsigned int pcieBaseAddrL,
		unsigned int pcieBaseAddrH) {
...
void set_direct_tx_dma(unsigned int devAddr, unsigned int pcieAddrH,
	...
	//xil_printf("set_direct_tx_dma %u\r\n");
...
void set_direct_rx_dma(unsigned int devAddr, unsigned int pcieAddrH,
		unsigned int pcieAddrL, unsigned int len) {
	HOST_DMA_CMD_FIFO_REG hostDmaReg;
	...
	//xil_printf("rx_dma_set_direct %u\r\n");
...
void set_auto_tx_dma(unsigned int cmdSlotTag, unsigned int cmd4KBOffset,
		unsigned int devAddr, unsigned int autoCompletion) {
	HOST_DMA_CMD_FIFO_REG hostDmaReg;
	...
	while ((g_hostDmaStatus.fifoTail.autoDmaTx + 1) % 256
			== g_hostDmaStatus.fifoHead.autoDmaTx)
		g_hostDmaStatus.fifoHead.dword = IO_READ32(HOST_DMA_FIFO_CNT_REG_ADDR);
	...
	hostDmaReg.autoCompletion = 0;
	//xil_printf("hostDmaReg.dword[0] =  %u\r\n", hostDmaReg.dword[0]);
	//xil_printf("hostDmaReg.dword[1] = %u\r\n", hostDmaReg.dword[1]);
	//xil_printf("hostDmaReg.dword[2] = %u\r\n", hostDmaReg.dword[2]);
	//xil_printf("hostDmaReg.dword[3] = %u\r\n", hostDmaReg.dword[3]);
	//xil_printf("auto_tx_dma_set_direct %u\r\n");
...
void set_auto_rx_dma(unsigned int cmdSlotTag, unsigned int cmd4KBOffset,
		unsigned int devAddr, unsigned int autoCompletion) {
	HOST_DMA_CMD_FIFO_REG hostDmaReg;
	unsigned char tempTail;
	...
	//xil_printf("rx_auto_dma_set_direct %u\r\n");
...
void check_direct_tx_dma_done() {
	while (g_hostDmaStatus.fifoHead.directDmaTx
			!= g_hostDmaStatus.fifoTail.directDmaTx) {
		g_hostDmaStatus.fifoHead.dword = IO_READ32(HOST_DMA_FIFO_CNT_REG_ADDR);
	}
}
...
void check_direct_rx_dma_done() {
	while (g_hostDmaStatus.fifoHead.directDmaRx
			!= g_hostDmaStatus.fifoTail.directDmaRx) {
		g_hostDmaStatus.fifoHead.dword = IO_READ32(HOST_DMA_FIFO_CNT_REG_ADDR);
	}
}
...
void check_auto_tx_dma_done() {
	while (g_hostDmaStatus.fifoHead.autoDmaTx
...
void check_auto_rx_dma_done() {
	while (g_hostDmaStatus.fifoHead.autoDmaRx
...
unsigned int check_auto_tx_dma_partial_done(unsigned int tailIndex,
	...
	if (g_hostDmaStatus.fifoHead.autoDmaTx
			== g_hostDmaStatus.fifoTail.autoDmaTx)
		return 1;

	if (g_hostDmaStatus.fifoHead.autoDmaTx < tailIndex) {
		if (g_hostDmaStatus.fifoTail.autoDmaTx < tailIndex) {
			if (g_hostDmaStatus.fifoTail.autoDmaTx
					> g_hostDmaStatus.fifoHead.autoDmaTx)
				return 1;
			else if (g_hostDmaAssistStatus.autoDmaTxOverFlowCnt
					!= (tailAssistIndex + 1))
				return 1;
		} else if (g_hostDmaAssistStatus.autoDmaTxOverFlowCnt
				!= tailAssistIndex)
...
unsigned int check_auto_rx_dma_partial_done(unsigned int tailIndex,
	...
	if (g_hostDmaStatus.fifoHead.autoDmaRx
			== g_hostDmaStatus.fifoTail.autoDmaRx)
		return 1;

	if (g_hostDmaStatus.fifoHead.autoDmaRx < tailIndex) {
		if (g_hostDmaStatus.fifoTail.autoDmaRx < tailIndex) {
			if (g_hostDmaStatus.fifoTail.autoDmaRx
					> g_hostDmaStatus.fifoHead.autoDmaRx)
				return 1;
			else if (g_hostDmaAssistStatus.autoDmaRxOverFlowCnt
					!= (tailAssistIndex + 1))
				return 1;
		} else if (g_hostDmaAssistStatus.autoDmaRxOverFlowCnt
				!= tailAssistIndex)
