...
//add by yanjie.tan
P_IN_NVM_FLAG_MAP inNvmFlagMapPtr;
...
void InitAddressMap() {
	...
	//add by yanjie.tan
	inNvmFlagMapPtr = (P_IN_NVM_FLAG_MAP) IN_NVM_FLAG_MAP_ADDR;
...
void ReadBadBlockTable(unsigned int tempBbtBufAddr[],
	...
	while (dataSize > 0) {
		for (dieNo = 0; dieNo < USER_DIES; dieNo++) {
			...
			SelectLowLevelReqQ(reqSlotTag, ReadBadBlockTableFUN);
		}
	...
	for (phyBlockNo = 0; phyBlockNo < TOTAL_BLOCKS_PER_DIE; phyBlockNo++) {
		for (dieNo = 0; dieNo < USER_DIES; dieNo++)
			if (!dieState[dieNo]) {
				...
				//SelectLowLevelReqQ(reqSlotTag);
				SelectLowLevelReqQ(reqSlotTag, FindBadBlockFUN);
		...
		for (dieNo = 0; dieNo < USER_DIES; dieNo++)
			if ((dieState[dieNo] == DIE_STATE_BAD_BLOCK_TABLE_NOT_EXIST)
				...
				if (loop == 0) {
					...
					//SelectLowLevelReqQ(reqSlotTag);
					SelectLowLevelReqQ(reqSlotTag, SaveBadBlockTableFUN);
				}
...
void EraseTotalBlockSpace() {
	...
	for (blockNo = 0; blockNo < TOTAL_BLOCKS_PER_DIE; blockNo++)
		for (dieNo = 0; dieNo < USER_DIES; dieNo++) {
			...
			//SelectLowLevelReqQ(reqSlotTag);
			SelectLowLevelReqQ(reqSlotTag, EraseTotalBlockSpaceFUN);
		}
	...
	for (blockNo = 0; blockNo < USER_BLOCKS_PER_DIE; blockNo++)
		for (dieNo = 0; dieNo < USER_DIES; dieNo++)
			if (!virtualBlockMapPtr->block[dieNo][blockNo].bad) {
				...
				SelectLowLevelReqQ(reqSlotTag, EraseUserBlockSpaceFUN);
				//SelectLowLevelReqQ(reqSlotTag);
...
// to do 目前NVM MAPPING没有限制大小，因此没有页面替换策略，以后待添加
void add_to_mpi(unsigned lpn) {
	//unsigned int virtualSliceAddr;
	unsigned char is_nvm_mapping;
	is_nvm_mapping = inNvmFlagMapPtr->inNvmFlag[lpn].in_nvm_flag;
	//virtualSliceAddr = logicalSliceMapPtr->logicalSlice[lpn].virtualSliceAddr;
	if (is_nvm_mapping != IN_NVM_MAPPING) {
		//logicalSliceMapPtr->logicalSlice[lpn].is_nvm_mapping = IN_NVM_MAPPING;
		inNvmFlagMapPtr->inNvmFlag[lpn].in_nvm_flag = IN_NVM_MAPPING;
	}
}
...
void EraseBlock(unsigned int dieNo, unsigned int blockNo) {
	...
	//SelectLowLevelReqQ(reqSlotTag);
	SelectLowLevelReqQ(reqSlotTag, EraseBlockFUN);
// block map indicated blockNo initialization
	...
	for (pageNo = 0; pageNo < USER_PAGES_PER_BLOCK; pageNo++) {
		...
		//add by yanjie.tan 初始化inNvmFlag
		inNvmFlagMapPtr->inNvmFlag[virtualSliceMapPtr->virtualSlice[virtualSliceAddr].logicalSliceAddr].in_nvm_flag =
				NOT_IN_NVM_MAPPING;
		virtualSliceMapPtr->virtualSlice[virtualSliceAddr].logicalSliceAddr =
				LSA_NONE;
...
unsigned int GetFromFbList(unsigned int dieNo, unsigned int getFreeBlockOption) //fb means free block
	...
	if (virtualBlockMapPtr->block[dieNo][evictedBlockNo].nextBlock
			!= BLOCK_NONE) {
