...
void InitReqPool() {
	...
	for (reqSlotTag = 0; reqSlotTag < AVAILABLE_OUNTSTANDING_REQ_COUNT;
			reqSlotTag++) {
		...
		//add by yanjie.tan init txId in reqPool
		reqPoolPtr->reqPool[reqSlotTag].txId = 0;
	}

	reqPoolPtr->reqPool[0].prevReq = REQ_SLOT_TAG_NONE;
	reqPoolPtr->reqPool[AVAILABLE_OUNTSTANDING_REQ_COUNT - 1].nextReq =
			REQ_SLOT_TAG_NONE;
...
unsigned int GetFromFreeReqQ() {
	...
	if (reqPoolPtr->reqPool[reqSlotTag].nextReq != REQ_SLOT_TAG_NONE) {
		...
		reqPoolPtr->reqPool[reqPoolPtr->reqPool[reqSlotTag].nextReq].prevReq =
				REQ_SLOT_TAG_NONE;
	} else {
...
void PutToBlockedByBufDepReqQ(unsigned int reqSlotTag) {
	...
	reqPoolPtr->reqPool[reqSlotTag].reqQueueType =
...
void SelectiveGetFromBlockedByBufDepReqQ(unsigned int reqSlotTag) {
	...
	} else if ((nextReq == REQ_SLOT_TAG_NONE)
			&& (prevReq != REQ_SLOT_TAG_NONE)) {
...
void PutToBlockedByRowAddrDepReqQ(unsigned int reqSlotTag, unsigned int chNo,
		unsigned int wayNo) {
	if (blockedByRowAddrDepReqQ[chNo][wayNo].tailReq != REQ_SLOT_TAG_NONE) {
		reqPoolPtr->reqPool[reqSlotTag].prevReq =
				blockedByRowAddrDepReqQ[chNo][wayNo].tailReq;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[blockedByRowAddrDepReqQ[chNo][wayNo].tailReq].nextReq =
				reqSlotTag;
...
void SelectiveGetFromBlockedByRowAddrDepReqQ(unsigned int reqSlotTag,
		unsigned int chNo, unsigned int wayNo) {
	unsigned int prevReq, nextReq;
...
void GetFromNandReqQ(unsigned int chNo, unsigned int wayNo,
		unsigned int reqStatus, unsigned int reqCode) {
	unsigned int reqSlotTag;
